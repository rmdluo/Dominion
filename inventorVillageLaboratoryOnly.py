...
class SingleGame:
    def __init__(self, numInventors, numVillages, numLabs):
        self.file = open("gameInventorVillageLaboratoryOnly.txt", "w")
        self.deck = []
        ...
        for card in range(numLabs):
            self.deck.append("action-laboratory")
        ...
        while(len(playableActions) > 0 and self.actions > 0):
            if("action-village" in playableActions):
                ...
                for number in range(2):
                    ...
                    self.draw(1)
                    card = self.hand[len(self.hand) - 1]
                    if(card[:6] == "action" and lenBefore != len(self.hand)):
                        playableActions.append(card)
    ...
    def buy(self):
        ...
        while(self.buys > 0):
            if(8 - self.costReduction <= self.coins):
                ...
                if(minNum < 10):
                    if(minNum == self.numLaboratory):
                        self.discard.append("action-laboratory")
